...
import {
  createProviderConnection, deleteProviderConnection, discoverProviderModels,
...
export function ProviderConnections() {
  ...
  const models = useMutation({ mutationFn: discoverProviderModels, onSuccess: (result) => setDraft((current) => ({ ...current, model: current.model || result.models[0] || "" })) });
  ...
  const edit = (connection: ProviderConnection) => { setEditing(connection.id); setDraft({ display_name: connection.display_name, provider_type: connection.provider_type, base_url: connection.base_url, model: connection.model, enabled: connection.enabled }); setMessage(null); };
  ...
  return <div data-testid="provider-connections" className="space-y-4 rounded-lg border bg-white p-6">
    ...
    <form className="grid gap-3 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
      ...
      <label>Model<input required value={draft.model} onChange={(e) => set("model", e.target.value)} /><button type="button" onClick={() => editing != null && models.mutate(editing)}>Discover models</button></label>
